...
from lightllm.distributed.device_communicators.custom_all_reduce_utils import gpu_p2p_access_check
from lightllm.distributed.parallel_state import in_the_same_node_as
from lightllm.utils.log_utils import init_logger
from lightllm.platforms import current_platform
from lightllm.utils.vllm_utils import cuda_device_count_stateless
...
def _can_p2p(rank: int, world_size: int) -> bool:
    for i in range(world_size):
        if i == rank:
            continue
        LIGHTLLM_SKIP_P2P_CHECK = os.getenv("LIGHTLLM_SKIP_P2P_CHECK", False)
        if LIGHTLLM_SKIP_P2P_CHECK:
...
class CustomAllreduce:
    ...
    def __init__(self, group: ProcessGroup, device: Union[int, str, torch.device], max_size=8192 * 1024) -> None:
        ...
        from lightllm.platforms.cuda import CudaPlatform
        ...
        # allreduce results.
        self.meta = torch.zeros(ops.meta_size() + max_size, dtype=torch.uint8, device=self.device)
        ...
        self._ptr = ops.init_custom_ar(self.meta, self.rank_data, handles, offsets, rank, self.full_nvlink)
