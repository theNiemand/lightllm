...
class TpPpPartBaseModel:
    ...
    def __init__(self, kvargs):
        ...
        self.nnodes = kvargs["nnodes"]
        self.pp_rank_ = kvargs["pp_rank"]
        self.world_size_ = kvargs["world_size"]

        self.tp_groups = kvargs.get("tp_groups")
        self.pp_groups = kvargs.get("pp_groups")

        self.first_stage = self.pp_rank_ == 0
        self.last_stage = self.pp_rank_ == (self.nnodes - 1)

        self.tp_group = self.tp_groups[self.pp_rank_]
        self.pp_prev_src = (self.pp_rank_ - 1) * self.world_size_ if self.pp_rank_ > 0 else (self.nnodes - 1) * self.world_size_
        self.pp_prev_group = self.pp_groups[self.pp_rank_ - 1] if self.pp_rank_ > 0 else self.pp_groups[-1]
        self.pp_succ_group = self.pp_groups[self.pp_rank_] if self.pp_rank_ < len(self.pp_groups) - 1 else self.pp_groups[-1]
        self.first_tp = self.tp_rank_ == 0
    ...
    def _init_config(self):
        ...
        self.pp_n_layer = self.config["n_layer"] // self.nnodes
        return
    ...
    def _init_weights(self):
        ...
        self.trans_layers_weight = [
            ...
            for i in range(self.pp_n_layer * self.pp_rank_, self.pp_n_layer * (self.pp_rank_ + 1))
        ]
        load_hf_weights(
            self.data_type,
    ...
    def _init_infer_layer(self):
        ...
        self.layers_infer = [
            self.transformer_layer_infer_class(
                i, tp_rank=self.tp_rank_, world_size=self.world_size_, network_config=self.config, group=self.tp_group, pp_rank=self.pp_rank_, mode=self.mode, 
            )
            for i in range(self.pp_n_layer)
        ]
        return
    ...
    def _context_forward(self, input_ids, infer_state: InferStateInfo):
        if self.nnodes <= 1:
            ...
            if not self.first_stage:
                input_embs = torch.empty((input_ids.shape[0], self.config["n_embed"]), dtype=self.data_type, device="cuda")
                dist.broadcast(input_embs, src=self.pp_prev_src, group=self.pp_prev_group)

            for i in range(self.pp_n_layer):
                input_embs = self.layers_infer[i].context_forward(input_embs, infer_state, self.trans_layers_weight[i])
            
            if self.first_tp and not self.last_stage:
                input_embs = input_embs.contiguous()
                dist.broadcast(input_embs, src=self.global_rank, group=self.pp_succ_group)
            ...
            if self.last_stage:
                ...
                if self.first_tp:
                    predict_logics = predict_logics.contiguous()
                    dist.broadcast(predict_logics, src=self.global_rank, group=self.pp_succ_group)
                return predict_logics

            if self.first_stage:
                predict_logics = torch.empty((infer_state.batch_size, self.vocab_size), dtype=self.data_type, device="cuda")
                dist.broadcast(predict_logics, src=self.pp_prev_src, group=self.pp_prev_group)
                return predict_logics
            return torch.empty((infer_state.batch_size, self.vocab_size), dtype=self.data_type, device="cuda")
