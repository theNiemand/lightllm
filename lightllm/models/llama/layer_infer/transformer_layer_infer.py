...
from lightllm.common.basemodel import TransformerLayerInferTpl, TransformerLayerInferTplOffload
...
class LlamaTransformerLayerInferOffload(TransformerLayerInferTplOffload):
    """
    """
    ...
    def _post_cache_kv(self, cache_k, cache_v, infer_state:LlamaInferStateInfo, layer_weight:LlamaTransformerLayerWeight):
        mem_manager = infer_state.mem_manager
        if infer_state.is_prefill:
            self._copy_kv_to_mem_cache(cache_k, cache_v, infer_state.prefill_mem_index, mem_manager, infer_state.offload_token_number)
        else:
            if not infer_state.decode_is_contiguous:
                self._copy_kv_to_mem_cache(cache_k, cache_v, infer_state.decode_mem_index, mem_manager, infer_state.offload_token_number)
    ...
    def _copy_kv_to_mem_cache(self, key_buffer, value_buffer, mem_index, mem_manager, offload_token_number):
        if self.mode == "":
            destindex_copy_kv(key_buffer, mem_index, mem_manager.key_buffer(self.layer_num_))
            destindex_copy_kv(value_buffer, mem_index, mem_manager.value_buffer(self.layer_num_))
        if self.mode == "int8kv":
